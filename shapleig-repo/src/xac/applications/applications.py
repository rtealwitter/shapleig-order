...
class ShapleyApplication(BaseApplication):
    ...
    def run_lazy_setup(
        ...
        if scalable_mode:
            ...
            if self.random_init_design:
                ...
                _temp_id_sampler.sample(init_design_size)
                init_design_samples_bool = torch.tensor(_temp_id_sampler.coalitions_matrix)
    ...
    def get_levgp_siq_value(
        ...
        # Record the design of the current (last-called) budget so the
        # selection-history analysis can read it; the final call leaves the
        # full-budget design in place.
        object.__setattr__(
            self,
            "archive_binary",
            torch.tensor(np.array(train_coals_bool), dtype=torch.float64),
        )
